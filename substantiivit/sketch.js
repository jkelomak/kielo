...
function draw() {
    ...
    if(currScreen === -1){
        ...
        /*
        for(var i = 0; i < t_taulukko.length; i++){
            for(j = 0; j < t_taulukko[0].length; j++){
                text(t_taulukko[i][j], 0 + 100*i, 100 + 3*j);
            }
        }
        */
        text(T_data.sanat[12],400,100);
        
        for(var i = 0; i < T_data.virheNimet.length; i++){
            text(T_data.virheNimet[i], 700 + 50*i, 30);
            console.log("wattt");
        }


        text(T_data_oikMaara(), 20, 20);
        for(var i = 0; i <  T_data.oikVektori.length; i++){
            text(T_data.oikVektori[i], 400, 100 + 30*i);
        }
